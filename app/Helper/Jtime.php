...
class Jtime
{
    ...
    public  static  function jTime($separator = "-")
    {
        ...
        $lol = "$jw $jd $jm $jy $time";


        $list = explode(' ', $lol);
        return  $list[3] . $separator .
            ($list[2] < 10? "0". $list[2]:$list[2]). $separator
            . ($list[1] <10?"0".$list[1]:$list[1]). " " . date("H:i:s");
    }
    public  static  function jTimeDate($separator = "-")
    {
        $datetime = date('Y-m-d-D H:i:s', time());
        $hastime = true;
        $localize = false;
